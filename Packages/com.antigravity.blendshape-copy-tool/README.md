...
- **Undo対応**: 操作ミスをしても `Ctrl + Z` で元に戻せます。
...
4. このパッケージのフォルダ（`com.antigravity.blendshape-copy-tool`）を選択します。
5. Unityに戻るとコンパイルが始まり、上部メニューの `Window > Avatar Tools > BlendShape Copy Tool` から利用可能になります。
...
2. **Source Mesh**: コピー元の SkinnedMeshRenderer をアサインします。
3. **Target Mesh**: コピー先の SkinnedMeshRenderer をアサインします。
4. **Copy BlendShapes** ボタンをクリックします。

### 2. 別のプロジェクトへのコピー（ファイル経由）
1. **コピー元プロジェクト**でウィンドウを開き、`Source Mesh` をセットして **Export to JSON** をクリックし、ファイルを保存します。
2. **コピー先プロジェクト**でウィンドウを開き、`Target Mesh` をセットして **Import from JSON** をクリックし、先ほど保存したファイルを選択します。

### 3. BlendShapeのリセット
- **Reset Target BlendShapes** ボタンをクリックすると、アサインされている `Target Mesh` のすべての値が0になります。
